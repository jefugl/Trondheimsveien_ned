...
def main():
    ...
    data = pd.DataFrame({
        ...
        "img": [inputdata.renna, inputdata.wembley, inputdata.buen, inputdata.valentino, inputdata.bella_notte,
                inputdata.perla, inputdata.pane_vino, inputdata.ocean, inputdata.schous]
    })

    m = folium.Map(location=start, width=800, height=900, zoom_start=14, min_zoom=10, max_zoom=18)
    ...
    for i in range(0, len(data)):
        folium.Marker(
            ...
            icon=DivIcon(html=f"""<div style="font-size: 12pt;font-family: helvetica;
                 color: blue">{data.iloc[i]['name']}</div>""")
        ).add_to(m)
